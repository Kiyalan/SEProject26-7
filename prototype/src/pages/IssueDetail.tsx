...
export default function IssueDetail() {
  ...
  const handleLlmEnhance = useCallback(async () => {
    if (!repoId || !issue || llmLoading) return
    setLlmLoading(true)
    try {
      const result = await analyzeIssueWithLlm(repoId, issue)
      setAnalysis(result)
    } finally {
      setLlmLoading(false)
    }
  }, [repoId, issue, llmLoading])
