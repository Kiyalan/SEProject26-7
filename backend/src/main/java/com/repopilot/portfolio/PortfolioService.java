package com.repopilot.portfolio;
...
public class PortfolioService {
    ...
    public PortfolioService(GitHubService github, JdbcTemplate jdbc) {
        this.github = github;
        this.jdbc = jdbc;
    }
    ...
    private Map<String, Map<String, Object>> localIndexMap() {
        List<Map<String, Object>> rows = jdbc.query(
                "SELECT repo_id, status, indexed_at, file_count, chunk_count FROM repo_index",
                (rs, rowNum) -> Map.of(
                        "repo_id", rs.getString("repo_id"),
                        "status", rs.getString("status"),
                        "indexed_at", rs.getString("indexed_at"),
                        "file_count", rs.getInt("file_count"),
                        "chunk_count", rs.getInt("chunk_count")
                )
        );
        Map<String, Integer> commitCounts = new HashMap<>();
        jdbc.query("SELECT repo_id, COUNT(*) AS c FROM repo_commits GROUP BY repo_id", rs -> {
            commitCounts.put(rs.getString("repo_id"), rs.getInt("c"));
        });

        Map<String, Map<String, Object>> result = new HashMap<>();
        for (Map<String, Object> row : rows) {
            String repoId = (String) row.get("repo_id");
            result.put(repoId, Map.of(
                    "indexed", "ready".equals(row.get("status")),
                    "indexedAt", Objects.toString(row.get("indexed_at"), ""),
                    "fileCount", row.get("file_count"),
                    "chunkCount", row.get("chunk_count"),
                    "commitSnapshots", commitCounts.getOrDefault(repoId, 0)
            ));
        }
        return result;
    }
